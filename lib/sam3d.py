...
class Sam3D(ABC):
    ...
    def __init__(self, args, cfg, xyz_min, xyz_max, cfg_model, cfg_train, \
        ...
        self.cfg = cfg
        self.args = args
        # 클래스 ID를 가져와서 YOLO 학습 인스턴스 설정
        self.target_class_id = int(cfg.yolo.target_class_id)
        try:
            self.target_class_name = model_yolo.names[self.target_class_id]
        except (KeyError, IndexError, TypeError) as exc:
            raise ValueError(
                f"Invalid YOLO class ID: {self.target_class_id}"
            ) from exc
        ...
        self.vsgflag = False
        self.matching_score = {}
        self.selected_instance_by_camera = {}
    ...
    def prompt_and_inverse(self, idx, HW, seg_m, yolo_m, dual_seg_m, depth, num_obj=1):
        H, W = HW[idx]
    ...
    def train_step(self, idx, sam_mask=None):
        ...
        if sam_mask is None:
            camera_id = int(self.data_dict['i_train'][idx])
            img = self.data_dict['images'][camera_id,:,:,:].numpy()
            img = utils.to8b(img)
            h, w, c = img.shape
            results = model_yolo.predict(source=img, imgsz=(h,w), classes=self.target_class_id)

            # --- 오류 방지를 위한 추가된 로직 시작 ---
            # YOLO가 객체를 감지하지 못했거나 마스크 데이터가 없는 경우
            if not results or not results[0].boxes or len(results[0].boxes) == 0 or results[0].masks is None or len(results[0].masks.data) == 0:
                print(f"Skipping view {idx} (camera {camera_id}): No objects detected or no masks generated by YOLO.")
                # 학습 단계를 건너뛰고 다음 뷰로 진행
                return utils.to8b(rgb.cpu().numpy()), None, idx >= len(render_poses)-1
            # --- 오류 방지를 위한 추가된 로직 끝 ---
            ...
            for mask, box in zip(results[0].masks.data, results[0].boxes):
                ...
                if torch.is_tensor(tmp_IoU):
                    tmp_IoU = tmp_IoU.item()
                iou_scores.append(float(tmp_IoU))
                yolo_confidences.append(float(box.conf.item()))
            # 종합 점수 계산
            combined_scores = [
                iou * 0.6 + confidence * 0.4
                for iou, confidence in zip(iou_scores, yolo_confidences)
            ]
            best_idx = int(np.argmax(combined_scores))
            best_matching_score = float(combined_scores[best_idx])

            # 신뢰도 출력 추가--------------------------------
            print(f"\n=== View {idx} ===")
            print(f"Detected instances: {len(results[0].boxes)}")
            for i, (confidence, matching_score) in enumerate(
                zip(yolo_confidences, combined_scores)
            ):
                print(
                    f"Instance {i}: Confidence={confidence:.4f}, "
            ...
            if self.vsgflag == False:
                self.matching_score[camera_id] = best_matching_score
                self.selected_instance_by_camera[camera_id] = best_idx
                print(f"Selected instance: {best_idx} "
                      f"(Confidence: {yolo_confidences[best_idx]:.4f}, "
                      f"Matching Score: {best_matching_score:.4f})")
    ...
    def prompting_coarse(self, H, W, seg_m, yolo_m, idx, index_matrix, num_obj):
        ...
        seg_m_for_prompt = yolo_m.unsqueeze(2)
        ...
        for num in range(num_obj):
            ...
            masks[selected] = yolo_m.cpu().numpy()
...
def fetch_seg_poses(seg_poses_type, data_dict):
    if seg_poses_type == 'train':
        ...
        # # by seok
        # render_poses = torch.cat([render_poses, render_poses], dim=0)
        # HW = np.concatenate((HW,HW),axis=0)
